"""
Streaming logic for converting Copilot stream to OpenAI format.

Converts CopilotEvent objects to OpenAI chat.completion.chunk SSE format.
"""

import json
import time
from typing import TYPE_CHECKING, AsyncGenerator, Callable, Awaitable, Dict, Any, Optional

import httpx
from fastapi import HTTPException
from loguru import logger

from copilot.utils import generate_completion_id, generate_tool_call_id
from copilot.config import FIRST_TOKEN_TIMEOUT, FIRST_TOKEN_MAX_RETRIES
from copilot.tokenizer import count_tokens, count_message_tokens, count_tools_tokens
from copilot.streaming_core import (
    parse_copilot_stream,
...
async def stream_copilot_to_openai_internal(
    ...
    """
    Internal generator for converting Copilot stream to OpenAI format.
    ...
    # Accumulate tool call chunks by index: {index: {id, type, function: {name, arguments}}}
    tool_calls_by_index: Dict[int, Dict[str, Any]] = {}

    try:
        async for event in parse_copilot_stream(response, first_token_timeout):
            ...
            elif event.type == "content" and event.reasoning_content:
                full_thinking_content += event.reasoning_content
            ...
            elif event.type == "tool_call" and event.tool_calls:
                # Accumulate incremental tool call chunks by index
                for tc_chunk in event.tool_calls:
                    idx = tc_chunk.get("index", 0)
                    if idx not in tool_calls_by_index:
                        tool_calls_by_index[idx] = {
                            "index": idx,
                            "id": tc_chunk.get("id", generate_tool_call_id()),
        ...
        finish_reason = "tool_calls" if tool_calls_by_index else "stop"
...
async def stream_copilot_to_openai(
    response: httpx.Response,
    model: str,
    model_cache: "ModelInfoCache",
    request_messages: Optional[list] = None,
    ...
    async for chunk in stream_copilot_to_openai_internal(
...
async def collect_stream_response(
    ...
    async for chunk_str in stream_copilot_to_openai(
        response, model, model_cache,
        request_messages=request_messages,
        request_tools=request_tools,
    ):
