"""
Streaming logic for converting Copilot stream to Anthropic Messages API format.

Formats CopilotEvent objects into Anthropic SSE events.
"""
...
from copilot.tokenizer import count_tokens, count_message_tokens
...
async def stream_copilot_to_anthropic(
    ...
    """
    Generator for converting Copilot stream to Anthropic SSE format.
    ...
    try:
        ...
        async for event in parse_copilot_stream(response, first_token_timeout):
            if event.type == "content" and event.content:
                ...
                # Close thinking block if transitioning to text content
                ...
                for tc_chunk in event.tool_calls:
                    ...
                    if func_chunk.get("name"):
                        existing["name"] += func_chunk["name"]
                    if func_chunk.get("arguments"):
                        existing["arguments"] += func_chunk["arguments"]

        # Close remaining text/thinking blocks
        if thinking_block_started and thinking_block_index is not None:
            yield _format_sse_event("content_block_stop", {
                "type": "content_block_stop", "index": thinking_block_index
            })
...
async def collect_anthropic_response(
    ...
    async for event in parse_copilot_stream(response):
        if event.type == "content" and event.content:
            full_content += event.content
        elif event.type == "content" and event.reasoning_content:
            full_thinking += event.reasoning_content
        elif event.type == "tool_call" and event.tool_calls:
            for tc_chunk in event.tool_calls:
                idx = tc_chunk.get("index", 0)
                if idx not in tool_calls_by_index:
